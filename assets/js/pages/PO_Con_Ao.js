    ...
    DS_MaTD = new kendo.data.DataSource({
        transport: {
            read: function (options) {
                $.ajax({
                    type: "POST",
                    url: "assets/ajax/Ajax_HopDong_CT_Ao.aspx",
                    data: {
...
function Ham_HienThi_Xuat_PO(value) {
    ...
    if (arr_dv[0] == "0") {
        return "<b>Số hợp đồng: " + arr_dv[1] + "</b><button style='margin-left:30px !important;margin-right:10px;' onclick='Ham_Xuat_DonHang( \" " + arr_dv[1] + " \");' type='button' class='btn btn-primary'>Xuất Thông báo Nhu cầu</button><a class='btn btn-info' onclick='Ham_Xuat_Ex(\" " + value + " \");'><i class='fa fa-download'></i> Xuất Excel</a>";
    }
    else {
